...
/**
 * Apply a patch in an isolated worktree, run tests, and create a draft PR.
 *
 * All external commands (git, npm, gh, fs) are injected via opts.mocks to
 * allow deterministic testing without side effects.
 *
 * @param {object} investigation
 * @param {object} fix
 * @param {object} config
 ...
 * @param {object} opts.mocks
 * @param {(args: string[]) => Promise<string>} opts.mocks.git
 * @param {(args: string[]) => Promise<string>} opts.mocks.npm
 * @param {(args: string[]) => Promise<string>} opts.mocks.gh
 * @param {{ writeFile: (path: string, content: string) => Promise<void>, remove: (path: string) => Promise<void> }} opts.mocks.fs
 * @returns {Promise<object>}
 */
export async function applyPatch(investigation, fix, config, opts) {
  const { repoRoot, worktreeRoot, issueId, mocks } = opts;
  const { git, npm, gh, fs } = mocks;

  const fingerprint8 = (investigation.location.file + fix.class)
    .split('')
  ...
  const branch = `forge/fix-${issueId}-${slugify(fix.class)}-${fingerprint8}`;
  const worktreeDir = `${worktreeRoot}/${branch.replace(/\//g, '-')}`;
  const patchFile = `${worktreeDir}/.forge-diff.patch`;

  try {
    // Create worktree on a new branch
    await git(['worktree', 'add', worktreeDir, '-b', branch]);

    // Write and apply the diff
    await fs.writeFile(patchFile, fix.diff);
    await git(['apply', patchFile], { cwd: worktreeDir });

    // Run tests per matching workspace
    const workspaces = config.project.workspaces ?? [];
    for (const ws of workspaces) {
      const wsDir = `${worktreeDir}/${ws.path}`;
      await npm(['install'], { cwd: wsDir });
      await npm(['run', 'build'], { cwd: wsDir });
      await npm(['run', 'typecheck'], { cwd: wsDir });
      await npm(['test'], { cwd: wsDir });
    }

    // Tests passed — stage, commit, push, create PR
    await git(['add', '-A'], { cwd: worktreeDir });
    await git(
      ['commit', '-m', `fix: ${fix.explanation ?? fix.class} [forge #${issueId}]`],
      { cwd: worktreeDir },
    );
    await git(['push', '-u', 'origin', branch], { cwd: worktreeDir });
    ...
    const prUrl = await gh([
      ...
      '--draft',
      '--title',
      `fix: ${fix.class} [forge #${issueId}]`,
      '--body',
      `Auto-generated by Forge for issue #${issueId}.\n\nFix class: ${fix.class}`,
      '--repo',
      config.project.github_repo,
    ]);

    return { status: 'pr_created', pr_url: prUrl.trim(), branch };
  } catch (err) {
    // Test or apply failure — delete branch, return rejection
    try {
